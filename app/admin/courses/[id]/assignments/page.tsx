    ...
    <div className="p-4 min-h-screen bg-gray-100">
      ...
      <ul className="max-w-md mx-auto space-y-4">
        {assignments.map((assignment) => (
          <li key={assignment.id} className="bg-white shadow-md rounded-lg p-4">
            {/* Include the courseId in the link path */}
            <Link href={`/admin/courses/${courseId}/assignments/assignment/${assignment.id}`} className="text-blue-600 hover:underline">
              {assignment.title}
            </Link>
          </li>
        ))}
