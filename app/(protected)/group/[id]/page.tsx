...
const DashboardNoAssignments = () => {
    ...
    const fetchFiles = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8000/api/groups/getallfiles/${courseID}`,
                {
                    method: "GET",
                    headers: { Authorization: `Bearer ${session.user.token}` },
                }
    ...
    const handleFileAdd = async (data: {
        ...
        try {
            const response = await fetch(
                `http://localhost:8000/api/groups/uploadfile/${courseID}`,
                {
                    method: "POST",
                    headers: { Authorization: `Bearer ${session?.user.token}` },
                    body: formData,
    ...
    const handleFileDelete = async (fileId: string) => {
        try {
            const response = await fetch(
                `http://localhost:8000/api/groups/deletefile/${courseID}/${fileId}`,
                {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${session?.user.token}` },
                }
...
export default dynamic(() => Promise.resolve(DashboardNoAssignments), {
    ssr: false,
});
