...
const SelectSuccessorPopup: React.FC<SelectSuccessorPopupProps> = ({
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            ...
            <DialogContent>
                <Typography>
                    As you are the creator, you must choose a successor before
                    leaving.
                </Typography>
                <FormControl fullWidth sx={{ mt: 2 }}>
                    <InputLabel id="successor-select-label">
                        Successor
                    </InputLabel>
                    <Select
        ...
        <Dialog open={open} onClose={onClose}>
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    variant="contained"
                    disabled={!title || !description || !selectedFile}
                >
                    Add File
                </Button>
            </DialogActions>
...
const DashboardNoAssignments = () => {
    ...
    const [activeTab, setActiveTab] = useState<"dashboard" | "people">(
        "dashboard"
    );
    ...
    const handleDeleteMember = async (memberId: string) => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/rmvuser/${courseID}/${memberId}`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const fetchMembers = async () => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/getallusers/${courseID}`,
                {
                    headers: { Authorization: `Bearer ${session.user.token}` },
                    method: "GET",
                }
    ...
    const fetchUserStatus = async () => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/getstatus/${courseID}/${session.user.id}`,
                {
                    headers: { Authorization: `Bearer ${session.user.token}` },
                    method: "GET",
                }
    ...
    const handleFileAdd = async (data: {
        title: string;
        description: string;
        file: File;
    }) => {
    ...
    const handleFileDelete = async (fileId: string) => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/deletefile/${courseID}/${fileId}`,
                {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${session?.user.token}` },
                }
    ...
    const handleFileUpdate = async (updatedData: {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/updatefile/${courseID}/${updatedData.fileId}`, // use updatedData.fileId here
                {
                    method: "POST",
                    headers: { Authorization: `Bearer ${session?.user.token}` },
                    body: formData,
    ...
    const handleLeaveGroup = async () => {
        ...
        if (currentUserRole === "Creator") {
            ...
            try {
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_URL}/api/groups/leavegrp/${courseID}`,
                    {
                        method: "DELETE",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const handleSuccessorSelected = async (successorId: string) => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/groups/leavegrp/${courseID}/${successorId}`,
                {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
