...
const DashboardPage = () => {
  ...
  const handleDeleteAssignment = async (assignmentId: number | string) => {
    ...
    if (!session) return;
    try {
      const response = await fetch(`http://localhost:8000/api/assgn/${assignmentId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${session.user.token}` },
      });
      if (response.ok) {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      if (session) {
        ...
        // Fetch announcements
        try {
          const response = await fetch(`http://localhost:8000/api/announce/${courseID}`, {
            headers: { Authorization: `Bearer ${token}` },
            method: "GET",
          });
          if (response.ok) {
  ...
  useEffect(() => {
    const fetchAttendanceStudents = async () => {
      try {
        ...
        const initAttendance = data.students.reduce((acc: { [key: string]: boolean }, student: Student) => {
        ...
        setAttError("Failed to load attendance students. Please try again later.");
      }
    };
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newAttendance = attStudents.reduce((acc: { [key: string]: boolean }, student: Student) => {
      acc[student._id] = false;
      return acc;
    }, {});
      ...
      await Promise.all(
        attStudents.map(async (student) => {
          try {
            ...
            console.error("Error fetching attendance summary for student", student._id, err);
            updatedAttendance[student._id] = false;
          }
        })
      );
    ...
    <Box sx={{ backgroundColor: "#F4F6F8", minHeight: "100vh", padding: 4 }}>
      ...
      <Grid container spacing={4} mb={4}>
        <Grid item xs={12} md={6}>
          ...
          {announcements.length > 4 && !announcementsLoading && (
            <Button onClick={() => setShowAllAnnouncements((prev) => !prev)} size="small">
        ...
        <Grid item xs={12} md={6}>
          ...
          {chapters.length > 4 && !chaptersLoading && (
            <Button onClick={() => setShowAllChapters((prev) => !prev)} size="small">
      ...
      <Box mt={4}>
        ...
        {assignmentsLoading ? (
          ...
          <Typography color="black">No assignments available</Typography>
        )}
      </Box>
      ...
      <Dialog open={feedbackDialogOpen} onClose={() => setFeedbackDialogOpen(false)} fullWidth maxWidth="sm">
        <Box>
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end", p: 2 }}>
            <Button onClick={() => setFeedbackDialogOpen(false)} color="secondary">
              Cancel
            </Button>
            <Button onClick={handleSubmitFeedback} variant="contained" color="primary" sx={{ ml: 1 }}>
              Submit Feedback
            </Button>
          </Box>
        </Box>
      ...
      {/* Attendance Dialog */}
      <Dialog open={openAttendanceDialog} onClose={() => setOpenAttendanceDialog(false)} fullWidth maxWidth="sm">
        <Box sx={{ p: 2 }}>
          ...
          <Box>
            {attStudents.map((student) => (
              <Box key={student._id} sx={{ display: "flex", alignItems: "center", mb: 1 }}>
                <input
                  type="checkbox"
          ...
          <Button onClick={handleSubmitAttendance} variant="contained" color="primary" sx={{ mt: 2 }} disabled={attLoading}>
