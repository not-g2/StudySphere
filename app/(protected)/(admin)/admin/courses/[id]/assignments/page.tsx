...
const AssignmentList: React.FC = () => {
    ...
    const handleUpdateDueDate = async (id: string, newDate: string) => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_URL}/api/assgn/${id}`,
                {
                    method: "PUT",
                    headers: {
            ...
            alert(
                "There was an error updating the due date. Please try again."
            );
        }
    };
    ...
    if (assignments.length === 0) {
        return (
            <p className="text-center text-gray-700">No assignments found.</p>
        );
    }
        ...
        <div className="p-4 min-h-screen bg-c2">
            ...
            <ul className="max-w-md mx-auto space-y-4">
                {assignments.map((assignment) => (
                    <li
                        key={assignment._id}
                        className="bg-c5 p-4 rounded-lg shadow-md"
                    >
            ...
            {selectedAssignment && (
                <DueDateModal
                    ...
                    onUpdateDueDate={(newDate) =>
                        handleUpdateDueDate(selectedAssignment._id, newDate)
                    }
                />
            )}
        </div>
