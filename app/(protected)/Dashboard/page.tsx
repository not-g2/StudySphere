  ...
  useEffect(() => {
    if (session) {
      ...
      fetch("http://localhost:8000/api/users/profile", {
        ...
        .then((res) => res.json())
        .then((data) =>
          setProfile({ xp: data.xp, level: data.level, auraPoints: data.auraPoints })
        )
      ...
      fetch(`http://localhost:8000/api/pomodoro/fetchuseranalytics/${session.user.id}`, {
        ...
        .catch((err) => console.error("Error fetching focus analytics:", err));
    }
  }, [session]);
      ...
      <div style={{ maxWidth: "1600px", margin: "0 auto", padding: "20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "20px" }}>
          ...
          >
            ...
            >
              <h3 style={{ margin: "0 0 10px", fontSize: "16px" }}>Aura Points</h3>
              <p style={{ fontSize: "20px", margin: 0 }}>{profile.auraPoints}</p>
            </div>
          </div>
