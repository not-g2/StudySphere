...
const SubjectTimeBarChart = ({ data }) => {
  ...
  data.forEach((item) => {
    const itemDay = new Date(item.date).toLocaleDateString("en-US", { weekday: "short" });
  ...
  data.forEach((item) => uniqueSubjects.add(item.subject));
  ...
  return (
    <div style={{ backgroundColor: "#333333", padding: "20px", borderRadius: "8px" }}>
      ...
      <div style={{ backgroundColor: "#333333", borderRadius: "8px", padding: "10px" }}>
        <ResponsiveContainer width="100%" height={400}>
          <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            {Array.from(uniqueSubjects).map((subject) => (
              <Bar key={subject} dataKey={subject} stackId="a" fill={subjectColorMap[subject]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
