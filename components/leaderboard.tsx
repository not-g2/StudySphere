...
const Leaderboard: React.FC<LeaderboardProps> = ({ session }) => {
  ...
  const isCurrentUserInTopFour = topFour.some((entry) => entry._id === currentUserId);
  ...
  if (isCurrentUserInTopFour) {
    ...
    const userEntry =
      leaderboardEntries.find((entry) => entry._id === currentUserId) ||
    ...
    <div className="p-4 bg-green-500 rounded-lg shadow-lg w-full">
      ...
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          <XAxis type="number" domain={[0, maxLevel]} tick={{ fill: "#fff" }} />
          <YAxis dataKey="name" type="category" width={100} tick={{ fill: "#fff" }} />
          <Tooltip />
