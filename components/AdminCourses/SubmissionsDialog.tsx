...
const SubmissionsDialog = ({
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogTitle sx={{ color: "black" }}>Submissions for: {selectedAssignmentTitle}</DialogTitle>
      <DialogContent dividers>
        {submissionsLoading ? (
          <Typography color="black">Loading submissions...</Typography>
        ) : submissions.length > 0 ? (
          submissions.map((submission: any) => (
            <Card key={submission._id} sx={{ backgroundColor: "#ffffff", padding: 2, marginBottom: 2 }}>
              ...
              <Button
                ...
                sx={{ marginTop: 1 }}
              >
                {submission.status === "graded" ? "Update Feedback" : "Mark as Corrected"}
              </Button>
            </Card>
          ))
        ) : (
          <Typography color="black">No submissions found for this assignment.</Typography>
        )}
      </DialogContent>
      <DialogActions>
